/**
 * Surcharge du client Socket.IO pour l'environnement Docker local.
 *
 * Ce fichier N'EST PAS le fichier d'origine du projet
 * (app/nodejs/client.js, qui pointe en dur vers
 * "http://www.interminale.fr.nf:3000", domaine de production
 * aujourd'hui hors service). Il est monté PAR-DESSUS celui-ci au
 * démarrage du conteneur web (voir docker-compose.yml), ce qui permet
 * de pointer vers le service "nodejs" exposé sur localhost:3000 SANS
 * modifier le code source du projet.
 *
 * Seule la ligne `io.connect(...)` change par rapport au fichier
 * d'origine ; tout le reste (logique chat / messages privés / demandes
 * d'amis) est strictement identique.
 */
(function($) {
